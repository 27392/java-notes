...
> 
> **在方法参数中的区别就是一个是限制了必须是我的子类,而另了一个限制必须是我的父类(通过两个简单的图就能看出)**
>
> **而在调用时区别就是上面说,一个能读一个能写(比较难理解,可以多看看实例代码加深理解)**

[参考资料](https://www.zhihu.com/question/20400700/answer/117464182)

### 8.8.3 无限定通配符

除了子类限定通配符和超类通配符,还可以使用无限定的通配符,例如

```java
Pair<?>
```

初看起来,这好像与原始的`Pair`类型一样.实际上,有很大的不同.类型`Pair<?>`有以下方法
...
`getFirst`方法的返回值只能赋给一个`Object`

`setFirst`方法不能被调用,甚至不能用`Object`调用

> Pair<?>与Pair的区别

`Pair<?>`表示持有某种特定类型,但是不知道具体是那种类型,也不能对它进行操作,因为不知道实际是那种类型,这是不安全的

`Pair`表示没有指定泛型参数,默认是`Object`,所以可以用任意`Object`对象调用原始`Pair`类的`方法,只不过编译器会有警告

> 无限定通配符用途

这样感觉这个类型并没有什么用,其实不然可以用它来做一些简单操作.

例如测试一个`Pair`对象是否包含一个`null`引用,它不需要实际的类型
