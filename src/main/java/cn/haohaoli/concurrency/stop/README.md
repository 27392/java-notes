...
| **static** boolean interrupted()   | 判断当前线程(即正在执行命令的线程)是否被中断|该方法是静态方法,调用后会将**当前线程**的中断状态重置为`false`|
...
1. 正确的停止线程不单单只是调用`interrupt`方法这么简单,同时还需要被中断线程配合.才能达到停止线程的目的

3. 在捕获`InterruptedException`异常时,不要去压制.可以选择将中断状态恢复,更好的选择是传递异常,让调用者处理

2. `sleep`方法会清除中断状态,所以在捕获其异常时应当在`ctach`子句中恢复中断状态,或则将异常向上传递

4. `interrupted`是静态方法,它检查当前线程是否被中断,同时清除该线程的中断状态.
   而`isInterrupted`则是实例方法,它用来检查线程是否终端,但调用该方法不会改变中断状态